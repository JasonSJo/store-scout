...
if not env파일.exists():
    ...
    elif not 토큰.startswith("eyJ"):
        # 터널 토큰은 base64 로 인코딩된 JSON 이라 항상 eyJ 로 시작한다.
        # 여기 걸리는 값은 대개 (a) 설치 명령줄을 통째로 붙여넣었거나
        # (b) 터널 ID·계정 API 토큰 같은 다른 값을 넣은 경우다.
        문제.append("토큰 형식이 다릅니다")
        if " " in 토큰:
            다음.append("설치 명령줄을 통째로 붙여넣은 것 같습니다 — "
                       "`cloudflared service install` 뒤의 eyJ... 부분만 넣으십시오")
        else:
            다음.append("터널 토큰은 eyJ 로 시작합니다. 터널 ID 나 계정 API 토큰을 "
                       "넣지 않았는지 보십시오 (Tunnels → 해당 터널 → Configure)")
        말("✕", f"토큰이 'eyJ' 로 시작하지 않습니다 (지금: {토큰[:10]}…)")
    else:
        말("✓", f"토큰 있음 ({len(토큰)}자)")
